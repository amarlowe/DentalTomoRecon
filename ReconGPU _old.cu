...
__global__ void ProjectionNorm(float * Norm, int view, float ex, float ey, float ez)
{
	...
	if ((i < d_Px) && (j < d_Py))
	{
		...
		float x1 = (1.0f - (float)d_Start_Z / ez) * dx1 + (d_Nx2 - 0.5f) + ex * d_PitchNx2;
		float y1 = (1.0f - (float)d_Start_Z / ez) * dy1 + (d_Ny2 - 0.5f) + ey * d_PitchNy2;
		float x2 = (1.0f - (float)d_Start_Z / ez) * dx2 + (d_Nx2 - 0.5f) + ex * d_PitchNx2;
		float y2 = (1.0f - (float)d_Start_Z / ez) * dy2 + (d_Ny2 - 0.5f) + ey * d_PitchNy2;
	...
	float *Error, int view, float ex, float ey, float ez)
{
	...
	if ((i < d_Px) && (j < d_Py))
	{
		...
		if (NP != 0)
		{
			...
			float x1 = (1.0f - (float)d_Start_Z / ez)
				* dx1 + (d_Nx2 - 0.5f) + ex * d_PitchNx2;
			float y1 = (1.0f - (float)d_Start_Z / ez)
				* dy1 + (d_Ny2 - 0.5f) + ey * d_PitchNy2;
			float x2 = (1.0f - (float)d_Start_Z / ez)
				* dx2 + (d_Nx2 - 0.5f) + ex * d_PitchNx2;
			float y2 = (1.0f - (float)d_Start_Z / ez)
				* dy2 + (d_Ny2 - 0.5f) + ey * d_PitchNy2;
...
template <bool ApplyWindow>
__global__ void BackProjectError(float * IM, float * IM2, float * Win,
	float beta, int view, float ex, float ey, float ez)
{
	...
	if ((i < d_Nx) && (j < d_Ny))
	{
		...
		float r = -ez / (d_Start_Z - ((float)k*d_PitchNz) - ez);
		...
		if (ApplyWindow == true) Window = Win[k];
		...
		for (int x = xx1; x < xx2; x++)
		{
			...
			for (int y = yy1; y < yy2; y++)
			{
				...
				val += tex2D(textError, (float)x + 0.5f, (float)y + 0.5f) *scale *Window;
				N += scale;
				ys = yend;
			}
			xs = xend;
...
__global__ void CorrectEdges(float * IM, float * IM2)
{
...
__global__ void GetGradIm(float * IM)
{
	...
	if ((i < d_Nx) && (j <d_Ny) && (k < d_Nz))
	{
		...
		IM[(j + k*d_MNy)*d_MNx + i] = sgn*sqrtf(Ix1*Ix1 + Iy1*Iy1 + Iz1*Iz1);// / 3.0f;
																			 //what does the sign of the gradient mean?
	}
...
void SetUpGPUMemory(struct SystemControl * Sys)
{
	...
	ChkErr(cudaMalloc((void**)&d_Window, sizeSlice));
	ChkErr(cudaMalloc((void**)&d_Window2, sizeSlice));
	...
	ChkErr(cudaMemset(d_Window, 0, sizeSlice));
	ChkErr(cudaMemset(d_Window2, 0, sizeSlice));
	...
	ChkErr(cudaMemcpyToSymbol(d_Start_Z, &Sys->Recon->Slice_0_z, sizeof(float)));
...
void GetReconNorm(struct SystemControl * Sys)
{
	...
	for (int view = 0; view < Sys->Proj->NumViews; view++)
	{
		...
		ProjectionNorm << < dimGridProj, dimBlockProj >> > (d_Norm, view, ex, ey, ez);
	}
	...
	AverageProjectionNorm << < dimGridProj, dimBlockProj >> > (d_Norm, d_NormCpy);
...
void LoadAndCorrectProjections(struct SystemControl * Sys)
{
	...
	for (int view = 0; view < Sys->Proj->NumViews; view++)
	{
		...
		ApplyGaussianBlurY << < dimGridProj, dimBlockProj, 0, streams[view] >> >
			(d_SinoBlurX, d_SinoBlurXY);

		ScatterCorrect << < dimGridProj, dimBlockProj, 0, streams[view] >> >
			(d_Sino, d_Proj, d_SinoBlurXY, view, log(40000.0f));
...
void AddTVandTVSquared(struct SystemControl * Sys)
{
	...
	DerivOfGradIm << < dimGridIm, dimBlockIm >> >	(d_DerivGradIm, eplison);

	GetGradIm << < dimGridIm, dimBlockIm >> > (d_GradIm);

	ChkErr(cudaBindTexture2D(NULL, textImage, d_GradIm, textImage.channelDesc,
		MemR_Nx, MemR_Ny*Sys->Recon->Nz, MemR_Nx * sizeof(float)));

	DerivOfGradIm << < dimGridIm, dimBlockIm >> >	(d_Image2, eplison);

	UpdateImageEstimate << < dimGridIm, dimBlockIm >> > (d_Image, d_DerivGradIm, d_Image2);
}
...
void ReconUsingSARTandTV(struct SystemControl * Sys)
{
	...
	dim3 dimBlockCorr(64, 4);
	dim3 dimGridCorr(Sys->Recon->Nx / 64 + Cx, Sys->Recon->Nz / 4);
...
/*
	for (int view = 0; view < Sys->Proj->NumViews; view++)
	{
		...
		ProjectImage << < dimGridSino, dimBlockSino >> >
			(d_Sino, d_Norm, d_Error, view, ex, ey, ez);

		ChkErr(cudaBindTexture2D(NULL, textError, d_Error, textImage.channelDesc,
			MemP_Nx, MemP_Ny, MemP_Nx * sizeof(float)));

		BackProjectError<false> << < dimGridIm, dimBlockIm >> >
			(d_Image, d_Image2, d_Window, Beta, view, ex, ey, ez);

		CorrectEdges << <dimGridCorr, dimBlockCorr >> >(d_Image, d_Image2);
...
//		std::cout << "Center slice is:" << centerSlice << std::endl;
	...
	int offset = Sys->Recon->Nz / 2;

	float * Window = new float[Sys->Recon->Nz];
	float * Window2 = new float[Sys->Recon->Nz];

	for (int k = 0; k < Sys->Recon->Nz; k++)
	{
		//		if (k < 23 + offset){
		//			Window[k] = 0.5f * (1.0f + cos(3.14159f*((2.0f*(float)(k - offset))
		//				/ (0.7213f*(60.0f)) - 1.0f)));
		//		}
		//		else if (k >= 38 + offset){
		//			Window[k] = 0.5f * (1.0f + cos(3.14159f*((2.0f*(float)(k - offset))
		//				/ (0.7213f*(60.0f)) - 2.0f / 0.7213f + 1.0f)));
		//		}
		//		else{
		//			Window[k] = 1.0f;
		//		}
		//	Window2[k] = exp(-((float)(k - (offset + 30)*(k - (offset + 30))) / (2.0f*100.0f)));
		Window[k] = 1.0f;
		Window2[k] = 1.0f;
	}
	ChkErr(cudaMemcpy(d_Window, Window,
		Sys->Recon->Nz * sizeof(float), cudaMemcpyHostToDevice));
	ChkErr(cudaMemcpy(d_Window2, Window,
		Sys->Recon->Nz * sizeof(float), cudaMemcpyHostToDevice));

	cudaMemset(d_Image, 0, sizeIm);
	float alpha = 1.0f;
	std::cout << "Window set and SART iterations starting" << std::endl;
	...
	for (int iter = 0; iter < 30; iter++)
	{
		...
		for (int view = 0; view <Sys->Proj->NumViews; view++)
		{
			...
			BackProjectError<true> << < dimGridIm, dimBlockIm >> >
				(d_Image, d_Image2, d_Window, Beta, view, ex, ey, ez);
...
//	float * Image = new float[MemR_Nx*MemR_Ny*Sys->Recon->Nz];

//	float * StdImage = new float[MemR_Nx*MemR_Ny];
	//std::cout << "Max Image and std image made" << std::endl;

//	for (int x = 0; x < MemR_Nx; x++) {
//		for (int y = 0; y < MemR_Ny; y++) {
//
//			float mean = (max(Image[(y + centerSlice*MemR_Ny)*MemR_Nx + x] - 0.025, 0.0)
//				+ max(Image[(y + (centerSlice - 1)*MemR_Ny)*MemR_Nx + x] - 0.025, 0.0)
//				+ max(Image[(y + (centerSlice + 1)*MemR_Ny)*MemR_Nx + x] - 0.025, 0.0)) / 5.0;
//
//			float std = 0;
//
//			for (int z = 0; z < Sys->Recon->Nz; z++) {
//				float val = max(Image[(y + z*MemR_Ny)*MemR_Nx + x] - 0.025, 0.0);
//				std += ((val - mean)*(val - mean)) / ((float)(Sys->Recon->Nz));
//
//			}
//			StdImage[y*MemR_Nx + x] = sqrt(std);

//		}
//	}
	//	std::cout << "Max Image and std image made 2" << std::endl;
	
//	float * d_StIm;
//	cudaMalloc((void**)&d_StIm, MemR_Nx*MemR_Ny * sizeof(float));
//	cudaMemcpy(d_StIm, StdImage, MemR_Nx*MemR_Ny * sizeof(float), cudaMemcpyHostToDevice);
	
	//CreateSyntheticProjectionNew << < dimGridSino, dimBlockSino >> >
	//	(d_Proj, d_Window, d_StIm, d_Image, log(40000.0f), centerSlice);
